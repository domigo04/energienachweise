...
VERBLEIBEND = {
    ...
    "fresh_water_station_present", "pipe_length_m",
}
...
ENTFERNT = {
    ...
    "floor_heating_pipe_m", "domestic_hot_water_included", "pump_count",
...
def test_kein_zweites_waermeerzeugerfeld():
    """Erzeugertypen liegen als bearbeitbare Systemliste ausserhalb der Features."""
    erzeuger = [k for k in LV_IMPORT_FEATURE_KEYS if k.startswith("generator")]
    assert erzeuger == ["generator_power_kw"]


def test_nur_erzeugerleistung_bleibt():
    assert "generator_type" not in LV_IMPORT_FEATURE_KEYS
...
def test_pumpenanzahl_ist_weg():
    assert "pump_count" not in LV_IMPORT_FEATURE_KEYS
...
def test_waermemessung_ist_weg():
    assert "heat_meter_count" not in LV_IMPORT_FEATURE_KEYS
    assert "heat_metering_present" not in LV_IMPORT_FEATURE_KEYS
